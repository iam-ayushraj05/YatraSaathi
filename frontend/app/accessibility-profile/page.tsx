          ...
          <div className="max-w-2xl mx-auto space-y-6">
            <div>
              <h1 className="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight flex items-center gap-2">
                <User className="h-6 w-6 text-violet-650 dark:text-violet-400" />
                {language === 'HI' ? 'पहुंच प्रोफ़ाइल सेटिंग्स' : 'Accessibility Profile Settings'}
              </h1>
              <p className="text-xs text-slate-400 dark:text-slate-500 font-medium mt-1">
                {language === 'HI' 
                  ? 'अपनी व्यक्तिगत गतिशीलता सहायता सुविधाओं और मार्ग दिशानिर्देशों को कॉन्फ़िगर करें।' 
                  : 'Configure your personal mobility assistance features and routing guidelines.'
                }
              </p>
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-6">
              ...
              <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-[#0b0a0f] p-6 shadow-sm space-y-4 transition-colors">
                <h3 className="font-bold text-slate-900 dark:text-slate-100 text-sm flex items-center gap-2 border-b border-slate-50 dark:border-slate-900 pb-3">
                  <Accessibility className="h-4.5 w-4.5 text-violet-650 dark:text-violet-400" />
                ...
                <div className="space-y-3">
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      ...
                      className="h-4.5 w-4.5 rounded border-slate-350 dark:border-slate-700 text-violet-600 focus:ring-violet-500 bg-transparent"
                    />
                    <div>
                      <p className="text-xs font-bold text-slate-850 dark:text-slate-200">{language === 'HI' ? 'सीढ़ियों से बचें' : 'Avoid Stairs'}</p>
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    />
                    <div>
                      <p className="text-xs font-bold text-slate-850 dark:text-slate-200">{language === 'HI' ? 'सीढ़ी-मुक्त मार्ग आवश्यक हैं' : 'Require Step-Free Paths'}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-550 font-medium">
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    />
                    <div>
                      <p className="text-xs font-bold text-slate-850 dark:text-slate-200">{language === 'HI' ? 'नियमित विश्राम स्टॉप को प्राथमिकता दें' : 'Prefer Regular Rest Stops'}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-550 font-medium">
                ...
                <div className="pt-2">
                  ...
                  <input
                    ...
                    min={50}
                    max={10000}
                    className="w-full rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-4 py-2.5 text-xs text-slate-850 dark:text-slate-200 focus:outline-none focus:border-violet-500"
                  />
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-[#0b0a0f] p-6 shadow-sm space-y-4 transition-colors">
                <h3 className="font-bold text-slate-900 dark:text-slate-100 text-sm flex items-center gap-2 border-b border-slate-50 dark:border-slate-900 pb-3">
                  <Compass className="h-4.5 w-4.5 text-violet-650 dark:text-violet-400" />
                ...
                <select
                  ...
                  className="w-full rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 py-3.5 px-4 text-xs font-medium text-slate-700 dark:text-slate-300 focus:border-violet-500 focus:outline-none appearance-none"
                >
              ...
              <div className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-[#0b0a0f] p-6 shadow-sm space-y-4 transition-colors">
                <h3 className="font-bold text-slate-900 dark:text-slate-100 text-sm flex items-center gap-2 border-b border-slate-50 dark:border-slate-900 pb-3">
                  <Eye className="h-4.5 w-4.5 text-violet-650 dark:text-violet-400" />
                ...
                <div className="space-y-3">
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    />
                    <div>
                      <p className="text-xs font-bold text-slate-850 dark:text-slate-200">{language === 'HI' ? 'हाई कंट्रास्ट इंटरफ़ेस' : 'High Contrast Interface'}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-550 font-medium">
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    />
                    <div>
                      <p className="text-xs font-bold text-slate-850 dark:text-slate-200">{language === 'HI' ? 'ब्रेल साइनेज वाले स्थानों को फ़िल्टर करें' : 'Filter places with Braille signage'}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-550 font-medium">
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    />
                    <div>
                      <p className="text-xs font-bold text-slate-850 dark:text-slate-200">{language === 'HI' ? 'ऑडियो वर्णनात्मक अलर्ट' : 'Audio Descriptive Alerts'}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-550 font-medium">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 bg-violet-600 text-white rounded-xl py-3.5 text-xs font-semibold hover:bg-violet-750 hover:shadow-lg disabled:opacity-60 transition-all"
              >
                {loading ? (
                  <>
